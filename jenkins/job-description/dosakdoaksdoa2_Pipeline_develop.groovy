...
def job_config = [
        job: [
                name: "dosakdoaksdoa2_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'dosakdoaksdoa2_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/dosakdoaksdoa2_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.dosakdoaksdoa2.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "dosakdoaksdoa2", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("dosakdoaksdoa2_Pipeline_develop");

job.with {

    authenticationToken('HBAs8MIdrRQgB8VLVCzBRGOAyERfAtCc')
